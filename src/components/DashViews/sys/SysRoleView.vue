<template>
    <v-container fill-height fluid grid-list-xl>
        <v-layout justify-center wrap>
            <v-flex md12>
                <div>
                    <material-card color="general" title="系统角色管理面板">
                        <v-btn color="general" class="mb-2" @click="dialog = true">创建新角色</v-btn>
                        <v-data-table :headers="headers" :items="sysRoleList" :rows-per-page-items="rowsAmount" :pagination.sync="pagination" :total-items="pagination.totalItems">
                            ...
                            <template v-slot:items="props">
                                ...
                                <td class="">{{ props.item.roleType }}</td>
                                <td class="">{{ new Date(props.item.createDate).toLocaleDateString() }}</td>
                                <td class="justify-center ">
                                    <v-icon medium class="mr-2" @click="settingItem(props.item)">settings</v-icon>
                        ...
                        <!--设置权限面板-->
                        <v-dialog v-model="settle_dialog" max-width="500">
                            <v-card>
                                <v-card-title primary-title class="headline">设置角色权限</v-card-title>
                                <v-divider light></v-divider>
                                <v-card-text>
                                    <v-container grid-list-md>
                                        <v-layout wrap>
                                            <template v-for="(item, index) in sysResourceList">
                                                <v-flex xs6 sm3>
                                                    <v-checkbox v-model="item.enable" :label="item.resourceName"></v-checkbox>
                                                </v-flex>
                                            </template>
                                        </v-layout>
                                    </v-container>
                                ...
                                <v-card-actions>
                                    <v-spacer />
                                    <v-btn color="blue darken-1" flat @click="settle_dialog = false">Cancel</v-btn>
...
export default {
    ...
    data: () => ({
        ...
        headers: [
            ...
            { text: '角色类型', value: 'roleType' },
            { text: '角色生成日期', value: 'createDate' }
        ],
        sysRoleList: [],
        sysResourceList: [],
        sysPermissionList: [],
        ...
        roleTypes: [
            { 'typeName': '管理员角色', 'typeValue': '0' },
            { 'typeName': '用户角色', 'typeValue': '1' }
        ],
    }),
    ...
    methods: {
        ...
        querySysRoleList() {
            ...
            this.$http.post('/sys/role/findAllByPage', {
                'page': page,
                'size': rowsPerPage
            }).then(response => {
        ...
        settingItem(item) {
            ...
            this.settingIndex = item.roleId;
            /*TODO 配置权限信息*/
        },

        settingItem1() {
            /*TODO 和原权限信息比较，生成新权限信息*/
        },

        /*弹出确认框*/
        deleteItem(item) {
            this.delete_dialog = true;
            this.deletedIndex = item.roleId;
            this.editedItem = Object.assign({}, item)
        },

        deleteItem1() {
            var _this = this;
            this.$http.delete('/sys/role/' + this.deletedIndex)
                .then(response => {
                    _this.snack = true;
